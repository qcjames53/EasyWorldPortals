...
# The height is a clone from -29 + 1(floor) + y_height + 1(y_height offset) = 
#   y_height - 27 (based on scratch area dimensions)
$scoreboard players set #height v $(y_width)
scoreboard players remove #height v 27
execute store result storage minecraft:portal height int 1 run scoreboard \
    players get #height v
