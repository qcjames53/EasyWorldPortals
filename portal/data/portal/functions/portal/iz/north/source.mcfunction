# Determine the boundaries of the illusion zone for north facing renders and 
#   copy the required blocks to player-space.
...
$execute positioned ~1.5 ~ ~1.5 positioned ~$(x_width) ~ ~ facing entity @p \
    feet positioned as @p rotated ~180 0 run \
    function portal:portal/iz/north/raycast_l
data modify storage minecraft:input side set value "r"
scoreboard players set #rc_depth v 80
execute positioned ~-0.5 ~ ~1.5 facing entity @p feet positioned as @p rotated \
    ~180 0 run function portal:portal/iz/north/raycast_r
